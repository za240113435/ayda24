...
def convertir_a_palabras(n):
    if n == 0:
        ...
        if resto == 0:
            return f"{convertir_a_palabras(millones)} millón" if millones == 1 else f"{convertir_a_palabras(millones)} millones"
        elif resto == 1:
            return f"Un millon" if millones == 1 else f"{convertir_a_palabras(millones)} millones"
        else:
            return f"{convertir_a_palabras(millones)} millón {convertir_a_palabras(resto)}" if millones == 1 else f"{convertir_a_palabras(millones)} millones {convertir_a_palabras(resto)}"
    elif n == 1000000000:
        return "mil millones"
